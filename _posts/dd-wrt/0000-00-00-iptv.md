...
1. Enable ssh:
* ![enable-ssh](/assets/img/dd-wrt/enable-ssh.jpg)

2. Enable jffs:
* ![enable-jffs](/assets/img/dd-wrt/enable-jffs.jpg)

3. Connect to ssh:
* ![connect-to-ssh](/assets/img/dd-wrt/connect-to-ssh.png)

4. Download udpxy for needed chip `Broadcom | Atheros` to `jffs` and set permissions:
*Note that `/jffs` is not cleaned after reboot, but `/tmp` is.*
...
6. Add command:
* ![add-command](/assets/img/dd-wrt/add-command.png)
